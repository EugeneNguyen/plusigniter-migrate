...
class Files {
	...
	{
		$path = (($is_full_path === FALSE) ? ($this->files_path . $filename . $this->suffix_path) : ($filename . $this->suffix_path) ) . $this->files_ext;
		if (!file_exists($path))
		{
			$path = (($is_full_path === FALSE) ? (APPPATH . 'development/' . $filename . $this->suffix_path) : ($filename . $this->suffix_path) ) . $this->files_ext;
		}
		return $path;
	}
	...
	protected function parse_file_data($data, $var_name, $files_save_like)
	{
		...
		if ($this->files_ext === '.php')
		{
			...
			if ($files_save_like !== 'lang' && !is_array($data))
			{
